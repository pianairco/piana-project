...
public class RouteClassGenerator {
    ...
    public static Set<Class<?>> generateAssetHandlerClasses(
            PianaServer pianaServer)
            throws Exception {
        Set<Class<?>> classes = new HashSet<>();
        AssetHandler[] assetHandlers = pianaServer.assetHandlers();

        if(assetHandlers == null || assetHandlers.length == 0)
            return classes;
        for(AssetHandler assetHandler : assetHandlers) {
            String className = null;
            try {
                className = getClassName(assetHandler.url());
...
//                        routerConfig, assetHandler.url(), className);
//                writeClassToFile(assetHandler.assetPath(),
...
//                .getHandlerClasses();
//        handlerClasses.stream().forEach(target -> {
//            Handler handler = AnnotationController
//                    .getHandler(target);
//            String className = null;
//            try {
//                className = getClassName(handler.baseUrl());
//
//                final String fullClassName = packageName
//                        .replace('.', '/')
//                        .concat("/")
//                        .concat(className);
//                StringBuilder classSource = createClassSource(
//                        routerConfig, handler.url(), className);
//                writeClassToFile(handler.assetPath(),
//                        className, classSource);
//                classes.add(registerClass(
//                        fullClassName, classSource));
//            } catch (Exception e) {
...
//            PianaServer pianaServer)
//            throws Exception {
//        String documentStartUrl = pianaServer.docStartUrl() == null ?
//                defaultDocumentStartUrl : pianaServer.docStartUrl();
